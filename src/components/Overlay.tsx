...
export default function Overlay({ scrollYProgress }: OverlayProps) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Unmount overlay text when the canvas section is scrolled out of view (past 1.8 screen heights)
      // This prevents the text from popping back in when scrollYProgress resets to 0 off-screen.
  ...
  return (
    <div className="absolute top-0 left-0 w-full h-full pointer-events-none z-10">
      <div className="sticky top-0 h-screen w-full flex flex-col justify-center px-6 sm:px-12 md:px-24">
        ...
        <motion.div style={{ opacity: opacity1, y: y1, scale: scale1 }} className="absolute inset-0 flex items-center justify-center text-white">
        ...
        <motion.div style={{ opacity: opacity2, y: y2, x: x2, scale: scale2 }} className="absolute inset-0 flex items-center justify-center md:justify-end px-6 sm:px-12 md:px-32 text-center md:text-right text-white">
        ...
        <motion.div style={{ opacity: opacity3, y: y3, x: x3, scale: scale3 }} className="absolute inset-0 flex items-center justify-center md:justify-start px-6 sm:px-12 md:px-32 text-center md:text-left text-white">
